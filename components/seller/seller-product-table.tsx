    ...
    <div className="bg-white border border-slate-200 rounded-xl shadow-xs flex flex-col gap-4 p-5">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          >
            <option value="all">{t("seller.products.allCategories")}</option>
            <option value="men">{t("categories.erkek")}</option>
            <option value="women">{t("categories.kadin")}</option>
            <option value="electronics">{t("categories.elektronik")}</option>
            <option value="shoes-bags">{t("categories.ayakkabi-canta")}</option>
            <option value="home-living">{t("categories.ev-yasam")}</option>
          </select>

          <Link href="/seller/dashboard/products/new">
            <Button variant="primary" size="sm" className="font-extrabold text-xs shrink-0">
              <Plus className="w-4 h-4 mr-1" />
              <span>{t("seller.products.newProduct")}</span>
            </Button>
          </Link>
        </div>
      </div>
        ...
        <table className="w-full text-left text-xs border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200 text-text-muted font-extrabold uppercase tracking-wider">
              <th className="p-3">{t("seller.products.thProduct")}</th>
              <th className="p-3">{t("seller.products.thCategory")}</th>
              <th className="p-3">{t("seller.products.thPrice")}</th>
              <th className="p-3">{t("seller.products.thStock")}</th>
              <th className="p-3">{t("seller.products.thStatus")}</th>
              <th className="p-3 text-right">{t("seller.products.thActions")}</th>
            </tr>
          </thead>
                ...
                <tr key={p.id} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="p-3 font-bold">
                    <span className={p.stock > 10 ? "text-emerald-700" : "text-amber-600"}>
                      {p.stock} {language === "en" ? "Units" : "Adet"}
                    </span>
                  </td>
                  <td className="p-3">
                    <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded text-[11px] font-extrabold">
                      {t("seller.products.statusActive")}
                    </span>
                  </td>
                  <td className="p-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Link href={`/product/${p.slug}`} title={t("seller.products.viewInStore")} className="p-1.5 text-slate-400 hover:text-primary">
                        <Eye className="w-4 h-4" />
                      </Link>
                      <Link href={`/seller/dashboard/products/${p.id}/edit`} title={t("seller.products.edit")} className="p-1.5 text-slate-400 hover:text-indigo-600">
